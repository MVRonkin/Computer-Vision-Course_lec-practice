...
def train_epoch(model, dataloader, optimizer, criterion, metrics, device, *,
    ...
    for i, (x, y) in enumerate(tqdm(dataloader, desc="Train", leave=False)):
        ...
        # Проверка меток для задачи классификации (если используется CrossEntropyLoss)
        if hasattr(criterion, 'reduction') and 'mean' in str(criterion):
            # Проверка на корректность меток (например, для CrossEntropyLoss)
            if y.dtype == torch.long:  # Предполагается CrossEntropyLoss
                num_classes = model.fc.out_features if hasattr(model, 'fc') else getattr(model, 'num_classes', None)
                if num_classes is not None:
                    assert y.min() >= 0 and y.max() < num_classes, f"Метки вне диапазона [0, {num_classes-1}]"
                else:
                    # Если num_classes не определен, хотя бы проверим, что метки >= 0
                    assert y.min() >= 0, f"Метки < 0: min={y.min().item()}"
            
            # Проверка на NaN/inf в метках
            assert not torch.isnan(y).any(), "NaN в метках y"
            assert not torch.isinf(y).any(), "inf в метках y"
        
        # Проверка на NaN/inf в данных
        assert not torch.isnan(x).any(), "NaN в данных x"
        assert not torch.isinf(x).any(), "inf в данных x"

        with torch.amp.autocast(device_type=device.type, enabled=use_amp):
            y_pred = model(x)
            
            # Проверка на NaN/inf в предсказаниях
            if torch.isnan(y_pred).any() or torch.isinf(y_pred).any():
                print(f"Найдены NaN/inf в y_pred на батче {i}")
        ...
        # --- Исправленный подсчет потерь ---
        # Умножаем на accumulation_steps только для отслеживания, но не для усреднения
        # Потому что loss уже был масштабирован в случае accumulation_steps > 1
        current_loss = loss.item()
        if accumulation_steps > 1:
            current_loss = current_loss * accumulation_steps  # Восстанавливаем масштаб
            
        epoch_loss += current_loss

        # Метрики (вычисляются без градиентов)
        with torch.no_grad():
            for name, fn in metrics.items():
                try:
                    metric_val = fn(y_pred, y).item()
                    if not (math.isnan(metric_val) or math.isinf(metric_val)):
                        epoch_metrics[name] += metric_val
                    else:
                        print(f"Предупреждение: метрика {name} = {metric_val}, пропущено")
                except Exception as e:
                    print(f"Ошибка вычисления метрики {name}: {e}")

    # Усреднение по количеству батчей
